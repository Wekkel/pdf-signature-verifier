...
{
    // UPDATE: Add RevokedSerialNumbers to cache
    public class EutlCacheData
    {
        public DateTime LastUpdated { get; set; }
        public DateTime HashLastUpdated { get; set; }
        public List<string> TrustedCertificatesB64 { get; set; }
        public Dictionary<string, string> CrlsB64 { get; set; }
        public List<string> RevokedSerialNumbers { get; set; } // NEW: Cache the hash itself
    }
    ...
    public class EutlService
    {
        ...
        public DateTime HashLastUpdated { get; private set; }
        ...
        // UPDATED: Load the hash directly from cache
        public string LoadFromCache()
        {
            try
            {
                if (File.Exists(_cacheFilePath))
                {
                    ...
                    // NEW: Load the cached hash
                    if (cacheData.RevokedSerialNumbers != null)
                    {
                        RevokedSerialNumbers = new HashSet<string>(cacheData.RevokedSerialNumbers);
                        HashLastUpdated = cacheData.HashLastUpdated;
                    }
        ...
        public async Task<string> UpdateTrustListAsync(Action<string> updateCallback)
        {
            try
            {
                using (var httpClient = new HttpClient())
                {
                    ...
                    {
                        count++;
                        updateCallback($"CRLs downloaden ({count}/{allCrlUrls.Count})...");
                        try
                        {
                    ...
                    updateCallback("Hash bouwen van ingetrokken certificaten...");
        ...
        // UPDATED: Save the hash to cache
        private void SaveToCache()
        {
            var cacheData = new EutlCacheData
            {
                LastUpdated = this.LastUpdated,
                HashLastUpdated = this.HashLastUpdated,
                ...
                RevokedSerialNumbers = this.RevokedSerialNumbers.ToList() // NEW: Save the hash
            };
        ...
        private void ProcessCrlsToBlacklist()
        {
            ...
            HashLastUpdated = DateTime.UtcNow;
        }

        public void RebuildHashFromCache()
        {
            ProcessCrlsToBlacklist();
            SaveToCache();
        }

        public bool HashNeedsUpdate(TimeSpan maxAge)
        {
            return !RevokedSerialNumbers.Any() ||
                   (DateTime.UtcNow - HashLastUpdated) > maxAge;
        }
    }
}